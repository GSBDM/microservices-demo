...
*/
  ...
  const response3 = await fetch("https://connect.squareupsandbox.com/v2/payments", {
    ...
    console.log("testing only for new fetch: " + JSON.stringify(jsonResp));
      globalVariable = jsonResp.payment.id;
    console.log("testing only for new fetch: globalVariable: " + JSON.stringify(globalVariable));
    return jsonResp;
    }
...
const result1 =  requestFunc().then(val => {
    ...
    console.log("testing only for new fetch: outside globalVariable: " + JSON.stringify(globalVariable));
    console.log("testing only for new fetch: outside globalVariable: " +  globalVariable);
  ...
  return { transaction_id: globalVariable};
  //return { err };
};
